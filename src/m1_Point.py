...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.y = y
        self.t += 1
        self.distance = (self.x - self.startx) + (self.y - self.starty)

    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.t += 1
        self.distance += math.sqrt(((self.x + dx) - self.x) ** 2 + ((self.y + dy) - self.y) ** 2)
    ...
    def get_distance_from_start(self):
        a = self.x - self.startx
        b = self.y - self.starty
        distance = math.sqrt(a ** 2 + b ** 2)
        return distance
    ...
    def closer_to(self, p2, p3):
        a = math.sqrt((p2.x - self.x) ** 2 + (p2.y - self.y) ** 2)
        b = math.sqrt((p3.x - self.x) ** 2 + (p3.y - self.y) ** 2)
        if a <= b:
            return p2
        if a >= b:
            return p3
